...
class Speedflip(Sequence):
    def __init__(self, player: PlayerData):
        ...
        if abs(car.position[0]) < 25:
            self.yaw_strength = 1
            self.drive_distance = 260
            self.initial_angle = np.pi / 22
        elif abs(car.position[0]) < 500:
            self.yaw_strength = 1
            self.drive_distance = 300
            self.direction = -self.direction
        else:
            self.yaw_strength = 1
            self.drive_distance = 220
    ...
    def get_action(self, player: PlayerData, game_state: GameState, prev_action: np.ndarray) -> List:
        ...
        if self.state == 'align':
            angle = np.arccos(np.clip(np.dot(car.forward(), initial_car.forward()), -1.0, 1.0))
        ...
        if self.state == 'cancel_flip':
            boost = 1 if np.linalg.norm(car.linear_velocity) < 2290 else 0
        ...
        print("Bot - this shouldn't be printed")
